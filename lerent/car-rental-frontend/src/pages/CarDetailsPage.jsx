...
// ... (existing imports, keep them but this tool only replaces chunks, so I will just import SEOHead at the top if needed, but since I can't easily add an import at the top without replacing the whole file header, I will assume I can just use the component if imported. Wait, I MUST import it. I will add the import at the top first.)

// Actually standard replace_file_content is better for contiguous blocks. I'll split this into two edits: adding import and adding usage. Or just one big block if possible.
...
import AudiS6Img from '../audis6.webp';
import MaseratiImg from '../maseratilevante.webp';
import BMW840iImg from '../bmw840i.webp';
import BMWX7Img from '../bmwx7.webp';
...
const getCarImages = (car) => {
  ...
  if (brand.includes('AUDI') && model.includes('A6')) return ['/src/audia6.webp'];
  if (brand.includes('BMW') && model.includes('540')) return ['/src/bmw540i.webp'];
  if (brand.includes('AUDI') && model.includes('S4')) return ['/src/audis4.webp'];
  if (brand.includes('AUDI') && model.includes('S6')) return ['/src/audis6.webp'];
  if (brand.includes('MASERATI')) return ['/src/maseratilevante.webp'];
  if (brand.includes('BMW') && model.includes('840')) return ['/src/bmw840i.webp'];
  if (brand.includes('BMW') && model.includes('X7')) return ['/src/bmwx7.webp'];
...
const CarDetailsPage = () => {
  ...
  // Construct Custom Schema for Car
  const carSchema = {
    "@context": "https://schema.org",
    "@type": "Product",
    "name": `${car.brand} ${car.model}`,
    "image": getCarImage(car),
    "description": getCarDescription(car.brand, car.model),
    "brand": {
      "@type": "Brand",
      "name": car.brand
    },
  ...
  return (
    <>
      <SEOHead
        title={`${car.brand} ${car.model} | Prenájom áut Nitra - Lerent`}
        description={`Prenájom ${car.brand} ${car.model} v Nitre. ${getCarDescription(car.brand, car.model).substring(0, 120)}... Cena od ${car.dailyRate}€/deň.`}
        image={getCarImage(car)}
        keywords={`prenájom ${car.brand} ${car.model}, požičovňa ${car.brand} Nitra, luxusné autá Nitra`}
        schema={carSchema}
      />
      <div className="min-h-screen bg-black text-white pb-20" style={{ fontFamily: 'AvantGarde, sans-serif' }}>
              ...
              <div className="absolute bottom-16 left-1/2 transform -translate-x-1/2 flex gap-2">
                {getCarImages(car).map((imageUrl, index) => (
                  <button
                    ...
                    className={`w-16 h-12 rounded overflow-hidden border-2 transition-all ${currentImageIndex === index ? 'border-white' : 'border-transparent opacity-70'
                      }`}
                  >
                    <img
                      src={imageUrl}
